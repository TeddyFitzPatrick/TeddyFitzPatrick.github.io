...
let canvas: HTMLCanvasElement,
    ...
    canvasX: number,
    canvasY: number,
    startRenderingButton: HTMLButtonElement;
// Mandelbrot variables
let divergenceIterations: number,
    zInitial: number,
    inverted: boolean,
    renderAxes: boolean,
    gradient: boolean,
...
export function Mandelbrot(){
    ...
    useEffect(() => {
        ...
        if (!canvas) throw new Error("Mandelbrot canvas not found");
        const ctx = canvas.getContext("2d");

        // canvas dimensions
        canvas.width = (window.innerWidth >= fullscreenSize) 
        ...
        canvas.addEventListener("click", (event)=>{
            const clickX = Number(event.clientX - canvasX);
            const clickY = Number(event.clientY - canvasY + window.scrollY);
...
// */
function f(zReal: number, zComplex: number, a: number, b: number) {
